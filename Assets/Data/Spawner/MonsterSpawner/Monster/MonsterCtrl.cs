...
public class MonsterCtrl : SecondMonoBehaviour
{
    ...
    private void LoadMonsterDrop()
    {
        if (this._monsterDrop != null) return;
        this._monsterDrop = transform.GetComponentInChildren<MonsterDrop>();
        Debug.LogWarning(transform.name + ": LoadMonsterDrop", gameObject);
    }
    private void LoadMonsterAnim()
    {
        if (this._monsterAnim != null) return;
        this._monsterAnim = transform.GetComponentInChildren<MonsterAnimation>();
        Debug.LogWarning(transform.name + ": LoadMonsterAnim", gameObject);
    }
